...
{
    int d1 = abs(a.xe-a.xs) + abs(a.ye-a.ys);
    int d2 = abs(b.xe-b.xs) + abs(b.ye-b.ys);
    if ( a.e==b.e ) { return ( d1>d2 ); } else { return (a.e<b.e); }
}
...
int fun( state c )
{
    ...
    for ( int i=1 ; i<=f ; i++ )
    {
        ...
        if ( mn>dis && dis + cars[i].t + real <= c.e && c.e<=t ) { mn = dis; id=i; }
    }
    return id;
}
...
int main()
{
    ...
    freopen("metropolis(output).txt" , "w" , stdout );
    cin>>r>>c>>f>>n>>b>>t;
    for ( int i=1 ; i<=n ; i++ )
    {
        int xs,ys,xe,ye,s,e;
